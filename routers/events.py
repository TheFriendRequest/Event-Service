...
def create_event(
    ...
    # Convert datetime objects to strings for JSON serialization
    if created_event.get('start_time') and isinstance(created_event['start_time'], datetime):
        created_event['start_time'] = created_event['start_time'].isoformat()
    if created_event.get('end_time') and isinstance(created_event['end_time'], datetime):
        created_event['end_time'] = created_event['end_time'].isoformat()
    if created_event.get('created_at') and isinstance(created_event['created_at'], datetime):
...
def update_event(
    ...
    # Convert datetime objects to strings for JSON serialization
    if updated_event.get('start_time') and isinstance(updated_event['start_time'], datetime):
        updated_event['start_time'] = updated_event['start_time'].isoformat()
    if updated_event.get('end_time') and isinstance(updated_event['end_time'], datetime):
        updated_event['end_time'] = updated_event['end_time'].isoformat()
    if updated_event.get('created_at') and isinstance(updated_event['created_at'], datetime):
